...
function World() {
  ...
  function init() {
    ...
    objects = [];
    stonePresenceProb = 0.2;
    maxstoneSize = 0.5;
    for (var i = 10; i < 40; i++) {
      createRowOfstones(i * -3000, stonePresenceProb, 0.5, maxstoneSize);
    }
  ...
  function loop() {
    // Update the game.
    if (!paused) {
      // Add more stones and increase the difficulty.
      if (objects[objects.length - 1].mesh.position.z % 3000 == 0) {
        ...
        if (difficulty % levelLength == 0) {
          ...
          switch (level) {
            case 1:
              stonePresenceProb = 0.45;
              maxstoneSize = 0.6;
              break;
            case 2:
              stonePresenceProb = 0.65;
              maxstoneSize = 0.85;
              break;
            case 3:
              stonePresenceProb = 0.85;
              maxstoneSize = 1.1;
              break;
            case 4:
              stonePresenceProb = 1;
              ...
              break;
            case 5:
              stonePresenceProb = 1.5;
              maxstoneSize = 2.0;
              break;
            case 6:
              stonePresenceProb = 2.55;
              maxstoneSize = 2.5;
              break;
            default:
              stonePresenceProb = 3.55;
              maxstoneSize = 3.25;
          }
        }
        ...
        createRowOfstones(-120000, stonePresenceProb, 0.5, maxstoneSize);
        scene.fog.far = fogDistance;
      }

      // Move the stones closer to the character.
      objects.forEach(function (object) {
        object.mesh.position.z += 200;
      });

      // Remove stones that are outside of the world.
  ...
  /**
   * Creates and returns a row of stones according to the specifications.
   *
   * @param {number} POSITION The z-position of the row of stones.
   * @param {number} PROBABILITY The probability that a given lane in the row
   *                             has a stone.
   * @param {number} MINSCALE The minimum size of the stones. The stones have a
   *							uniformly distributed size from minScale to maxScale.
   * @param {number} MAXSCALE The maximum size of the stones.
   *
   */
  function createRowOfstones(position, probability, minScale, maxScale) {
    for (var lane = -1; lane < 2; lane++) {
      var randomNumber = Math.random();
      if (randomNumber < probability) {
        var scale = minScale + (maxScale - minScale) * Math.random();
        var stone = new Stone(lane * 800, -400, position, scale);
        objects.push(stone);
...
function Character() {
  ...
  this.shortsColor = Colors.yellow;
...
/**
 * A collidable stone in the game positioned at X, Y, Z in the scene and with
 * scale S.
 */
function Stone(x, y, z, s) {
  // Explicit binding.
  var self = this;

  // The object portrayed in the scene.
  this.mesh = new THREE.Object3D();
  //  var top = createCylinder(1, 550, 550, 5, Colors.black, 0, 1000, 0);
  //  var mid = createCylinder(1, 550, 550, 5, Colors.black, 0, 800, 0);
  var bottom = createCylinder(2, 550, 550, 5, Colors.brownDark, 0, 500, 0);
  //  var trunk = createCylinder(1, 550, 550, 5, Colors.black, 0, 125, 0);
  //  this.mesh.add(top);
  // this.mesh.add(mid);
  ...
  /**
   * A method that detects whether this stone is colliding with the character,
   * which is modelled as a box bounded by the given coordinate space.
   */
  this.collides = function (minX, maxX, minY, maxY, minZ, maxZ) {
    var stoneMinX = self.mesh.position.x - this.scale * 250;
    var stoneMaxX = self.mesh.position.x + this.scale * 250;
    var stoneMinY = self.mesh.position.y;
    var stoneMaxY = self.mesh.position.y + this.scale * 1150;
    var stoneMinZ = self.mesh.position.z - this.scale * 250;
    var stoneMaxZ = self.mesh.position.z + this.scale * 250;
    return (
      stoneMinX <= maxX &&
      stoneMaxX >= minX &&
      stoneMinY <= maxY &&
